...
void ChaineDeSortie::InfToSuf(string expression) {
    ...
    for (int i = 0; i < expression.length()+1; i++) { //parcourir l'expression infixe
        ...
        if (n->type == 'r') {
            ...
            this->ajouter(n); //on ajoute directement la variable à la chaine de sortie
        }else if (!p.vide()) {
            if (getopPriority(expression[i]) ==
                ...
                while (!p.vide() &&  getopPriority(expression[i]) < getopPriority(
                        p.get()->ope)) { // tant que l'opérateur au sommet de la pile est strictement plus prioritaire que celui de l'expression
